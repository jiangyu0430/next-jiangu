...
export default function ProjectDetailClient({ params }: any) {
  // Unwrap params promise
  const resolvedParams: { slug: string | string[] } = React.use(params)
  ...
  useEffect(() => {
    ...
    setIsIntersectingList(Array(projectContent.images.length).fill(false))
  ...
  // projects is now imported at the top
            ...
            <div className="flex items-center justify-between gap-2">
              <div
                className={`font-semibold text-black dark:text-white transition-all duration-300 overflow-hidden text-ellipsis line-clamp-2 ${
                  isSticky ? 'text-2xl' : 'text-2xl lg:text-3xl'
                }`}
              >
                {project.title}
              </div>
            ...
            >
              <div className="text-base text-zinc-400 flex items-center mt-4 mb-2">
                ...
                {project.type && project.date && (
                  <div className="w-[6px] h-[6px] rounded-full bg-zinc-300 dark:bg-zinc-500 mx-2" />
                )}
