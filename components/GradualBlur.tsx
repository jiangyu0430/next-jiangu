...
type GradualBlurProps = PropsWithChildren<{
  ...
  scrollDynamicHeight?: number

  /**
   * The scroll progress threshold at which the blur effect starts.
   * Value between 0 and 1. Scroll progress below this threshold will result in no blur.
   */
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const [dynamicHeightRem, setDynamicHeightRem] = useState(0)
  ...
  useEffect(() => {
    if (config.scrollDynamicHeight === undefined) return
    ...
    const onScroll = () => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        // Calculate scroll progress and apply scrollStartThreshold to delay blur start
        const threshold = config.scrollStartThreshold ?? 0
        let progress = docHeight > 0 ? Math.min(scrollTop / docHeight, 1) : 0

        if (progress < threshold) {
          progress = 0
        } else {
          progress = (progress - threshold) / (1 - threshold)
        }

        setDynamicHeightRem(progress * config.scrollDynamicHeight)
      })
    }
  ...
  }, [config.scrollDynamicHeight, config.scrollStartThreshold])
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      baseStyle.height =
        config.scrollDynamicHeight !== undefined
          ? `${dynamicHeightRem}rem`
          : responsiveHeight
    ...
    return baseStyle
  }, [config, responsiveHeight, responsiveWidth, isVisible, dynamicHeightRem])
