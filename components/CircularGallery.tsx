...
class App {
  ...
  constructor(
    ...
    this.mediasImages = (items && items.length ? items : [])
      .map((item) => {
        ...
        return '' // fallback 避免 never 类型
      })
      .filter((img) => {
        if (!img) return false
        const lower = img.split('?')[0].toLowerCase()
        return !/\.(gif|mp4)$/.test(lower)
      })
  ...
  createRenderer() {
    ...
    this.container.appendChild(this.gl.canvas as any)
  }
  createCamera() {
  ...
  destroy() {
    ...
    if (
      ...
      this.renderer.gl.canvas.parentNode.removeChild(
        this.renderer.gl.canvas as any
      )
    }
  }
}
...
export default function CircularGallery({
  ...
  useEffect(() => {
    const images: string[] = notesImages
      .map((item: string | { image: string }) => {
